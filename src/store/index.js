...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    SET_MOBILE_PHONE: (state, mobilePhoneNumber) => {
      state.mobilePhoneNumber = mobilePhoneNumber
    },
    SET_TIME: (state,createDate) => {
      ...
      return new Promise((resolve, reject) => {
        getUserInfo(state.token).then(data => {
          if (data.success) {
            ...
            commit('SET_MOBILE_PHONE',data.data.mobilePhoneNumber)
            commit('SET_TIME' ,data.data.createDate)
            resolve(data)
          } else {
            commit('SET_ACCOUNT', '')
            commit('SET_NAME', '')
